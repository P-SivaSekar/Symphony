...
const idx1 = content.indexOf(searchStart.replace(/\$code/g, '$' + 'code'));
...
if (idx1 !== -1 && idx2 !== -1) {
  ...
  Future<Playlist?> createPlaylist(String name, {bool isGlobal = false}) async {
    ...
    try {
`.replace(/\$code/g, '$' + 'code').replace(/\$e/g, '$' + 'e').replace(/\$_user/g, '$' + '_user');

  const newContent = content.substring(0, idx1) + correctBlock + content.substring(idx2);
